...
> **Agents: do not edit this file.** It is the humans' instructions to you, not
> a scratchpad. Never add, reword, or delete anything here as a side effect of
> another task, however relevant it seems at the time. Edit it only when a human
> has asked you, in that conversation, to change AGENTS.md itself. If you think
> a rule belongs here, say so in your reply and let them decide.
...
- **Don't write a new field on the session-creation path in the release that
  adds it.** `hasOnly` rejects the *whole* write over one unknown key, and a
  browser on a cached bundle runs ahead of the deployed rules, so that turns a
  new field into a broken "New session" button. Default it to absent-means-X and
  let a user action mint it (`requestsOpen` in `ui/sync.js`).
